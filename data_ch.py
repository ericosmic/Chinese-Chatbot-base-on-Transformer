...
def prepro_ch(data):
    result_data = list()
    seqfilter=re.compile(' ')
    
    for seq in tqdm(data) :
        seq=seqfilter.sub('',seq)
        if seq != '' and seq != ' ' and seq != '\n' and seq != '?':
            i = jieba.cut(seq)
...
def data_tokenizer(data):
    ...
    for sentence in data:
        ...
        sentence = re.sub(CHANGE_FILTER, "", str(sentence))
...
def load_vocabulary():
    ...
    if (not (os.path.exists(DEFINES.vocabulary_path))):
        ...
        if (os.path.exists(DEFINES.data_path)):
            ...
            # 질문과 답에 대한 열을 가져 온다.
            question, answer = list(data_df['Q']), list(data_df['A'])
            if DEFINES.tokenize_as_morph:  # 형태소에 따른 토크나이져 처리 根据语素进行标记处理
                question = prepro_ch(question)
                answer = prepro_ch(answer)
            data = []
            # 질문과 답변을 extend을 
            # 통해서 구조가 없는 배열로 만든다.
